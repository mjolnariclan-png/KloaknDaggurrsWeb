const KD = (() => {
  ...
  function gameCard(c){
    ...
    return `<a class="game-card rarity-${esc(c.rarity.toLowerCase())} ${open?"":"classified-card"} reveal" href="#/card/${encodeURIComponent(c.slug)}">
      ...
      <div class="card-art"><img src="${esc(c.image||"assets/img/card-back.png")}" alt=""></div>
  ...
  function factionsPage(){
    return `<section class="page-hero"><p class="eyebrow">THE NINE</p><h1>FACTIONS</h1><p>Allegiances are chosen. Motives are hidden. Victory belongs to those who understand both.</p></section><section class="section"><div class="faction-grid">${data.factions.map(factionCard).join("")}</div></section>`;
  }
...
&gt; CODE INPUT: READY_</pre><form id="vault-code-form"><input id="vault-code" autocomplete="off" placeholder="ENTER VAULT CODE"><button class="btn primary">UNLOCK</button></form><p id="vault-message"></p></div></section><section class="section"><div id="vault-grid" class="vault-grid">${files}</div></section>`;
  ...
  function learnPage(){
    return `<section class="page-hero"><p class="eyebrow">FIELD MANUAL 01</p><h1>LEARN KLANDESTINE</h1><p>Know the rules. Hide your allegiance. Never assume the table is telling you the truth.</p></section><section class="section"><div class="rule-steps"><article class="rule-card"><span>01</span><h2>DRAW</h2><p>Build options before the table understands what you are holding.</p></article><article class="rule-card"><span>02</span><h2>DECEIVE</h2><p>Use Runes and hidden information to make certainty impossible.</p></article><article class="rule-card"><span>03</span><h2>MANEUVER</h2><p>Attack, bargain, pressure, or let someone else do the damage.</p></article><article class="rule-card"><span>04</span><h2>FULFILL</h2><p>Every faction wants something. Win before the table learns exactly what.</p></article></div></section><section class="section manual-copy"><h2>Setup</h2><p>Klandestine begins with hidden allegiance. Prepare the deck, establish the players, distribute required faction information, and keep private information private.</p><h2>Card Types</h2><p>Primordials (kings), Vigor (mana), Accoutrements (equipment), Runes (spells), and Creatures form the foundation of your strategy.</p><h2>Victory</h2><p>Victory is faction-dependent. Everyone is trying to win. Nobody is required to tell you how.</p></section>`;
  }
