const KD = (() => {
  ...
  function gameCard(c){
    ...
    return `<a class="game-card rarity-${esc((c.rarity||"Common").toLowerCase())} ${open?"":"classified-card"} reveal" href="#/card/${encodeURIComponent(c.slug)}">
      <div class="foil"></div><div class="card-top"><span>#${String(c.card_number??c.number??0).padStart(3,"0")}</span><span>${esc(c.rarity)}</span></div>
      <div class="card-art"><img src="${esc(c.image_url||c.image||"assets/img/card-back.svg")}" alt=""></div>
      <div class="card-copy"><span>${esc(c.faction_name||f?.name||"UNKNOWN")}</span><h3>${esc(open?c.name:"██████████")}</h3><small>${esc(c.card_type||c.type)}</small>${countdown(c)}</div></a>`;
  }
  ...
  function home(){
    return `${data.site.intro&&!localStorage.getItem("kd-intro-v5")?`<div id="intro" class="intro-sequence"><img src="assets/img/brand-mark.svg" alt=""><p>SIGNAL ACQUIRED</p><h2>KLOAK &amp; DAGGURRS</h2><span>CLICK / TAP TO ENTER</span></div>`:""}
    <section class="hero"><div class="hero-grid"></div><div class="hero-sigil"><img src="assets/img/brand-mark.svg" alt=""></div><div class="hero-copy reveal"><p class="eyebrow">${esc(data.site.eyebrow||"KLOAK & DAGGURRS PRESENTS")}</p><h1>${esc(data.site.title||"KLANDESTINE")}</h1><p class="hero-line">${esc(data.site.line1)}<br><strong>${esc(data.site.line2)}</strong></p><div class="hero-actions"><a class="btn primary" href="#world">Enter the Shadows</a><a class="btn ghost" href="#/learn">Learn Klandestine</a></div>${backendBadge()}</div></section>
  ...
  function factionsPage(){
    return `<section class="page-hero"><p class="eyebrow">THE THIRTEEN</p><h1>FACTIONS</h1><p>Allegiances are chosen. Motives are hidden.</p></section><section class="section"><div class="faction-grid">${data.factions.map(factionCard).join("")}</div></section>`;
  }
...
&gt; CODE INPUT: READY_</pre>${session?.user?`<form id="vault-code-form"><input id="vault-code" autocomplete="off" placeholder="ENTER VAULT CODE"><button class="btn primary">UNLOCK</button></form>`:`<a class="btn primary" href="#/login">Sign In to Enter Codes</a>`}<p id="vault-message"></p></div></section><section class="section"><div class="vault-grid">${files}</div></section>`;
  ...
  function learnPage(){
    return `<section class="page-hero"><p class="eyebrow">FIELD MANUAL 01</p><h1>LEARN KLANDESTINE</h1><p>Know the rules. Hide your allegiance.</p></section><section class="section"><div class="rule-steps"><article class="rule-card"><span>01</span><h2>DRAW</h2><p>Draw 2 cards.</p></article><article class="rule-card"><span>02</span><h2>ACT</h2><p>Play up to 2 cards, place a face-down Kloak, or discard.</p></article><article class="rule-card"><span>03</span><h2>DECEIVE</h2><p>Defenders may flip Kloaks. Bluffing is part of the fight.</p></article><article class="rule-card"><span>04</span><h2>END</h2><p>End your turn with no more than 7 cards in hand.</p></article></div></section><section class="section manual-copy"><h2>Field Notes</h2><p>Klandestine uses a 120-card deck for 3–6 players and is designed around hidden 1-of-13 faction allegiances, unique abilities, secret win conditions, calculated deception and betrayal. Typical play time is roughly 20–30 minutes.</p><h2>Card Rarities</h2><p>Common, Uncommon, Rare, Mythic and Gold.</p></section>`;
  }
